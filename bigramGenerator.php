...
//SELECT DISTINCT `gram` , sum( frequency ) f, length( gram ) FROM `ngramfrequency` WHERE `n` =2 GROUP BY  gram ORDER BY f DESC

$stmt="Select distinct word,sum(frequency) f from $summary_table_prefix$lang where length(word)>=1 group by word order by f desc";
echo $stmt;
connect();
...
{	
	...
	if(count($char_array)>=1)
	{
		for($i=0;$i<(count($char_array)-1);$i++){
			echo "\t".$char_array[$i].$char_array[$i+1];
			$insert="Insert into ngramfrequency(gram,n,frequency) values('".$char_array[$i].$char_array[$i+1]."',2,$freq) on duplicate key update frequency=frequency+".$freq;
